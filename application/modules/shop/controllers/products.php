...
class Products extends CI_Controller {	
	...
	function index($order_by="name", $sort_order="ASC", $code=0, $page=0, $rows=15)
	{
		...
		$data['page_title']	= lang_key('products');
		...
		$data['content'] 	= load_view('products',$data,TRUE);
	...
	function bulk_save()
	{
		...
		if(!$products)
		{
			$this->session->set_flashdata('error',  lang_key('error_bulk_no_products'));
		...
		$this->session->set_flashdata('message', lang_key('message_bulk_update'));
		redirect($this->config->item('admin_folder').'/products');
	}
	...
	function form($id = false, $duplicate = false)
	{
		...
		$data['page_title']		= lang_key('product_form');
		...
		{	
			...
			if (!$product)
			{
				$this->session->set_flashdata('error', lang_key('error_not_found'));
				redirect($this->config->item('admin_folder').'/products');
			}
		...
		*/
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$data['content'] 	= load_view('product_form',$data,TRUE);
		...
		else
		{
			...
			$this->session->set_flashdata('message', lang_key('message_saved_product'));
	...
	function delete($id = false)
	{
		...
		{	
			...
			else
			{
				...
				$this->session->set_flashdata('message', lang_key('message_deleted_product'));
				redirect($this->config->item('admin_folder').'/products');
			}
		}
		else
