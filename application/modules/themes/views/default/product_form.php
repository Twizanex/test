...
{
	if(confirm('<?php echo lang_key('confirm_remove_image');?>'))
	{
		var id	= img.attr('rel')
		$('#gc_photo_'+id).remove();
	}
...
{
	if(confirm('<?php echo lang_key('confirm_remove_option');?>'))
	{
		$('#option-'+id).remove();
	}
}
...
<div class="row">
	<div class="span8">
		<div class="tabbable">
			<ul class="nav nav-tabs">
				<li class="active"><a href="#product_info" data-toggle="tab"><?php echo lang_key('details');?></a></li>
				<?php //if there aren't any files uploaded don't offer the client the tab
				if (count($file_list) > 0):?>
				<li><a href="#product_downloads" data-toggle="tab"><?php echo lang_key('digital_content');?></a></li>
				<?php endif;?>
				<li><a href="#product_categories" data-toggle="tab"><?php echo lang_key('categories');?></a></li>
				<li><a href="#product_options" data-toggle="tab"><?php echo lang_key('options');?></a></li>
				<li><a href="#product_related" data-toggle="tab"><?php echo lang_key('related_products');?></a></li>
				<li><a href="#product_photos" data-toggle="tab"><?php echo lang_key('images');?></a></li>
			</ul>
		</div>
		<div class="tab-content">
			<div class="tab-pane active" id="product_info">
				<div class="row">
					<div class="span8">
						<?php
						$data	= array('placeholder'=>lang_key('name'), 'name'=>'name', 'value'=>set_value('name', $name), 'class'=>'span8');
				...
				<div class="row">
					<div class="span8">
						<label><?php echo lang_key('excerpt');?></label>
						<?php
				...
				<div class="row">
					<div class="span8">
						<fieldset>
							<legend><?php echo lang_key('inventory');?></legend>
							<div class="row" style="padding-top:10px;">
								<div class="span3">
									<label for="track_stock"><?php echo lang_key('track_stock');?> </label>
									<?php
								 	$options = array(	 '1'	=> lang_key('yes')
														,'0'	=> lang_key('no')
														);
								...
								<div class="span3">
									<label for="fixed_quantity"><?php echo lang_key('fixed_quantity');?> </label>
									<?php
								 	$options = array(	 '0'	=> lang_key('no')
														,'1'	=> lang_key('yes')
														);
								...
								<div class="span2">
									<label for="quantity"><?php echo lang_key('quantity');?> </label>
									<?php
				...
				<div class="row">
					<div class="span8">
						<fieldset>
							<legend><?php echo lang_key('header_information');?></legend>
							<div class="row" style="padding-top:10px;">
								<div class="span8">
									
									<label for="slug"><?php echo lang_key('slug');?> </label>
									<?php
									$data	= array('name'=>'slug', 'value'=>set_value('slug', $slug), 'class'=>'span8');
									echo form_input($data);?>
									
									<label for="seo_title"><?php echo lang_key('seo_title');?> </label>
									<?php
									$data	= array('name'=>'seo_title', 'value'=>set_value('seo_title', $seo_title), 'class'=>'span8');
									echo form_input($data);
									?>

									<label for="meta"><?php echo lang_key('meta');?> <i><?php echo lang_key('meta_example');?></i></label> 
									<?php
			...
			<div class="tab-pane" id="product_downloads">
				<div class="alert alert-info">
					<?php echo lang_key('digital_products_desc'); ?>
				</div>
				<fieldset>
					<table class="table table-striped">
						<thead>
							<tr>
								<th><?php echo lang_key('filename');?></th>
								<th><?php echo lang_key('title');?></th>
								<th style="width:70px;"><?php echo lang_key('size');?></th>
								<th style="width:16px;"></th>
							</tr>
						</thead>
						<tbody>
						<?php echo (count($file_list) < 1)?'<tr><td style="text-align:center;" colspan="6">'.lang_key('no_files').'</td></tr>':''?>
				...
				<div class="row">
					<div class="span8">
						<?php if(isset($categories[0])):?>
							<label><strong><?php echo lang_key('select_a_category');?></strong></label>
							<table class="table table-striped">
							    <thead>
									<tr>
										<th colspan="2"><?php echo lang_key('name')?></th>
									</tr>
								</thead>
							<?php
					...
					<?php else:?>
						<div class="alert"><?php echo lang_key('no_available_categories');?></div>
					<?php endif;?>
					</div>
				</div>
			</div>
			...
			<div class="tab-pane" id="product_options">
					...
					<div class="span8">
						<div class="pull-right" style="padding:0px 0px 10px 0px;">
							<select id="option_options" style="margin:0px;">
								<option value=""><?php echo lang_key('select_option_type')?></option>
								<option value="checklist"><?php echo lang_key('checklist');?></option>
								<option value="radiolist"><?php echo lang_key('radiolist');?></option>
								<option value="droplist"><?php echo lang_key('droplist');?></option>
								<option value="textfield"><?php echo lang_key('textfield');?></option>
								<option value="textarea"><?php echo lang_key('textarea');?></option>
							</select>
							<input id="add_option" class="btn" type="button" value="<?php echo lang_key('add_option');?>" style="margin:0px;"/>
						</div>
					</div>
				</div>
				...
				$(document).ready(function(){
					...
					$('body').on('click', '.delete-option-value', function(){
						if(confirm('<?php echo lang_key('confirm_remove_value');?>'))
						{
			...
			<div class="tab-pane" id="product_related">
				<div class="row">
					<div class="span8">
						<label><strong><?php echo lang_key('select_a_product');?></strong></label>
					</div>
				</div>
				<div class="row">
					<div class="span2" style="text-align:center">
						...
						<div class="row">
							<div class="span2" style="margin-top:8px;">
								<a href="#" onclick="add_related_product();return false;" class="btn" title="Add Related Product"><?php echo lang_key('add_related_product');?></a>
							</div>
						</div>
					</div>
					<div class="span6">
	...
	<div class="span4">
		<?php
	 	$options = array(	 '0'	=> lang_key('disabled')
							,'1'	=> lang_key('enabled')
							);
...
function add_image($photo_id, $filename, $alt, $caption, $primary=false)
{
	...
	<div class="row gc_photo" id="gc_photo_<?php echo $photo_id;?>" style="background-color:#fff; border-bottom:1px solid #ddd; padding-bottom:20px; margin-bottom:20px;">
		...
		<div class="span6">
			<div class="row">
				<div class="span2">
					<input name="images[<?php echo $photo_id;?>][alt]" value="<?php echo $alt;?>" class="span2" placeholder="<?php echo lang_key('alt_tag');?>"/>
				</div>
				<div class="span2">
					<input type="radio" name="primary_image" value="<?php echo $photo_id;?>" <?php if($primary) echo 'checked="checked"';?>/> <?php echo lang_key('primary');?>
				</div>
				<div class="span2">
					<a onclick="return remove_image($(this));" rel="<?php echo $photo_id;?>" class="btn btn-danger" style="float:right; font-size:9px;"><i class="icon-trash icon-white"></i> <?php echo lang_key('remove');?></a>
				</div>
			</div>
			<div class="row">
				<div class="span6">
					<label><?php echo lang_key('caption');?></label>
		...
		<td>
			...
			<div class="option-form" id="option-form-<?php echo $count;?>">
				<div class="row-fluid">
					...
					<div class="span10">
						<input type="text" class="span10" placeholder="<?php echo lang_key('option_name');?>" name="option[<?php echo $count;?>][name]" value="<?php echo $po->name;?>"/>
					</div>
					
					<div class="span2" style="text-align:right;">
						<input class="checkbox" type="checkbox" name="option[<?php echo $count;?>][required]" value="1" <?php echo ($po->required)?'checked="checked"':'';?>/> <?php echo lang_key('required');?>
					</div>
				</div>
				<?php if($po->type!='textarea' && $po->type!='textfield'):?>
				<div class="row-fluid">
					<div class="span12">
						<a class="btn" onclick="add_option_value(<?php echo $count;?>);"><?php echo lang_key('add_item');?></a>
					</div>
				</div>
				<?php endif;?>
				<div style="margin-top:10px;">
					...
					<div class="row-fluid">
						...
						<div class="span1">&nbsp;</div>
						<?php endif;?>
						<div class="span3"><strong>&nbsp;&nbsp;<?php echo lang_key('name');?></strong></div>
						<div class="span2"><strong>&nbsp;<?php echo lang_key('value');?></strong></div>
						<div class="span2"><strong>&nbsp;<?php echo lang_key('weight');?></strong></div>
						<div class="span2"><strong>&nbsp;<?php echo lang_key('price');?></strong></div>
						<div class="span2"><strong>&nbsp;<?php echo ($po->type=='textfield')?lang_key('limit'):'';?></strong></div>
					</div>
...
function add_related_product()
{
	...
	else
	{
		...
		{
			alert('<?php echo lang_key('alert_select_product');?>');
		}
		else
		{
			alert('<?php echo lang_key('alert_product_related');?>');
		}
	}
}
...
{
	if(confirm('<?php echo lang_key('confirm_remove_related');?>'))
	{
	...
	return '
			<tr id="related_product_'.$id.'">
				<td>
					...
					'.$name.'</td>
				<td>
					<a class="btn btn-danger pull-right btn-mini" href="#" onclick="remove_related_product('.$id.'); return false;"><i class="icon-trash icon-white"></i> '.lang_key('remove').'</a>
				</td>
			</tr>
		';
}